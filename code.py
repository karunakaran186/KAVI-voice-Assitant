...
def screen_record():                                                                   #Screen Recording function              
    ...
    save_file = os.path.join(f_path,"output.avi")                                      #saves the file in the corresponding directory

    s_size = (1920,1080)
    fps = 10.0                                                                         #determines how fast the slow the playback speed will be

    fcc = cv2.VideoWriter_fourcc(*"mp4v")
    out = cv2.VideoWriter("output.avi",fcc,fps,(s_size))
...
def run_kavi():
    ...
    elif 'screen recording' in command:                                # Screen Recroding command                         
