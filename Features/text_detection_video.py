...
def run_inference(image):
	...
	scores = interpreter.get_output_details()[0]['index']
	geometry = interpreter.get_output_details()[1]['index']

	# perform inference and parse the outputs
	interpreter.set_tensor(input_details[0]['index'], image)
	interpreter.invoke()
	scores_resized = interpreter.get_tensor(scores)
	geometry_resized = interpreter.get_tensor(geometry)
	scores_resized = np.transpose(scores_resized, (0, 3, 1, 2))
	geometry_resized = np.transpose(geometry_resized, (0, 3, 1, 2))

	return (scores_resized, geometry_resized)
